...
void drawStuff() {
    ...
    glUniform1f(glGetUniformLocation(shader->Program, "portion"), fraction);
...
void setupOpenGLDrawingCode() {
	...
	setupTexture(&texture2, "awesomeface.png");
        shader = new Shader("tutorial4_part2.vs", "tutorial4_excercise4.frag");
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode) {
	if(key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        ...
        if(fraction < 1.0f) { fraction += 0.1f; }
    } else if(key == GLFW_KEY_DOWN && action == GLFW_PRESS) {
        if(fraction > 0.0f) { fraction -= 0.1f; }
    }
}
